...
tuner = RandomSearch(
    ...
    project_name='journey_hyperparameter_tuning')  # name of the project
...
new_data = {
    ...
    'position': 'Unknown'  # The position is unknown for prediction
}

new_text = np.array([new_data['disability_type'] + ' ' + new_data['skills_one'] + ' ' + new_data['skills_two'] + ' ' + new_data['position']])

new_sequences = tokenizer.texts_to_sequences(new_text)
new_sequences = pad_sequences(new_sequences, maxlen=max_seq_length)

predictions = model.predict(new_sequences)
predicted_label_indices = np.argsort(predictions[0])[::-1][:10]  # Select top 10 indices
ranked_labels = [unique_labels[index] for index in predicted_label_indices]

print("Top 10 Predicted Label Ranking:")
for rank, label in enumerate(ranked_labels):
    print(f"Rank {rank+1}: {label}")
